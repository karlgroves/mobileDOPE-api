...
router.post(
  ...
  validate([
    ...
    body('name').trim().notEmpty().withMessage('Ammo name is required').isLength({ max: 255 }),
    body('manufacturer')
      .trim()
      .notEmpty()
    ...
    body('powder_type').optional().trim().isLength({ max: 100 }),
    body('powder_weight')
      .optional()
      .isFloat({ min: 0 })
...
router.put(
  ...
  validate([
    ...
    body('manufacturer').optional().trim().notEmpty().isLength({ max: 255 }),
    body('bullet_weight').optional().isFloat({ min: 0.01, max: 1000 }),
    body('bullet_type').optional().trim().notEmpty().isLength({ max: 100 }),
    body('ballistic_coefficient_g1').optional().isFloat({ min: 0, max: 1 }),
    body('ballistic_coefficient_g7').optional().isFloat({ min: 0, max: 1 }),
    body('muzzle_velocity').optional().isFloat({ min: 0.01, max: 5000 }),
    body('powder_type').optional().trim().isLength({ max: 100 }),
    body('powder_weight').optional().isFloat({ min: 0 }),
    body('lot_number').optional().trim().isLength({ max: 100 }),
    body('notes').optional().trim().isLength({ max: 5000 }),
  ]),
